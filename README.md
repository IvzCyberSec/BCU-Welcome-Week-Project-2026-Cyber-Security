...
|---:|---|---|
| 23 | Telnet | Remote administrative access |
| 81 | HTTP | Camera web interface |

The exact Nmap output may vary depending on the camera and lab configuration.

### Student questions

1. What services have been discovered?
2. Why might Telnet be a security concern?
3. Why should the web management interface be investigated?
4. Which services are actually required for the camera's intended purpose?
...
1. Scan the complete lab subnet.
2. Display discovered hosts.
3. Allow the student to select a target.
4. Run service/version enumeration against the selected target.
5. Present the results in a beginner-friendly format.
